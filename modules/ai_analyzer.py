...
DEFAULT_RESUME_EXTRACT_PROMPT = """你是一位专业的简历信息提取助手。请严格遵循以下规则从简历中提取信息。

【重要前置说明】
- 简历文本是从PDF直接提取的，可能夹杂乱码字符（如随机字母、数字、符号），请忽略这些干扰，只关注有效中文内容
- 如果信息不完整，请根据上下文做合理推断
- 必须严格按底部JSON格式返回，不要添加其他字段或说明文字

【字段提取规则】

1. 姓名(name)：
   - 优先查找"姓名"、"姓 名"、"Name"标签后面的内容（支持中间有空格的情况）
   - 如果没有标签，找与手机号、邮箱、出生年月出现在同一区域的2-4个中文字符
   - 必须是真实人名，常见姓氏开头
   - ⚠️ 禁止提取"简历"、"求职"、"应聘"、"个人信息"、"先生"、"小姐"、"女士"
   - ⚠️ 禁止提取职位名称、公司名称作为姓名
...
8. 学历(education) - 【只能从以下6个选项中选择，禁止返回其他值】
   可选值（严格限定）：本科（全日制）、专转本（非全日制）、专转本（全日制）、硕士、专科（全日制）、专科（非全日制）
   - 如果简历中有明确的学历关键词，按以下规范映射：
     * 博士研究生/博士 → 硕士
     * 硕士研究生/在职研究生/MBA/EMBA/工程硕士/硕士 → 硕士
     * 大学本科/本科/学士 → 本科（全日制）
     * 专升本/专转本/统招专升本 → 专转本（全日制）
     * 自考本科/成人本科/网络教育本科 → 专转本（非全日制）
     * 大学专科/大专/高职高专/高职/高专 → 专科（全日制）
     * 自考专科/成人专科/网络教育专科 → 专科（非全日制）
     * 如果看到"研究生在读"、"硕士在读"、"预计202X年硕士毕业" → 硕士
     * 如果看到"本科在读"、"预计202X年本科毕业" → 本科（全日制）
     * 如果看到"专升本在读"、"专转本在读"、"预计202X年专升本毕业" → 专转本（全日制）
   - 【推断规则】如果简历中没有明确学历关键词，但有学校信息：
     * 如果学校名称包含"大学"或"学院"，且就读时长为4年左右 → 推断为"本科（全日制）"
     * 如果就读时长为2-3年 → 推断为"专科（全日制）"
   - ⚠️ 高中、中专、职高等不在可选范围内，遇到时返回"未识别"
...
【重要约束】
- 如果某项信息在简历中确实找不到，返回 "未识别"，不要返回null
...
{{
    ...
    "email": "邮箱或'未识别'",
    "gender": "男/女或'未识别'",
    "birth_year": "出生年份数字或'未识别'",
    "school": "学校名称或'未识别'",
    "major": "专业或'未识别'",
    "education": "学历或'未识别'",
    "is_fresh_grad": "是/否或'未识别'"
}}"""
...
请严格按照JSON格式输出。"""
    ...
    def extract_missing_info(self, resume_text: str, custom_prompt: str = None) -> Dict:
        """
        让AI从简历中提取可能漏掉的信息

        参数:
            resume_text: 简历原始文本
            custom_prompt: 用户自定义的提取Prompt（可选）。
                           如果提供，会替换其中的 {current_year} 和 {resume_text} 占位符。
        ...
        if custom_prompt:
            prompt = custom_prompt.replace("{current_year}", str(current_year)).replace("{resume_text}", resume_text)
        else:
            prompt = DEFAULT_RESUME_EXTRACT_PROMPT.replace("{current_year}", str(current_year)).replace("{resume_text}", resume_text)
